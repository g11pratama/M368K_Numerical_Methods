...
class Matrix {
 ...
 public:
  ...
  ///return a Vector of column j of this matrix
  Vector getCol(int j) const;  
  ///sets column j of this matrix to vector j
  void setCol(const Vector& x, int j);
  ///converts this matrix into an identity matrix
  void identity();
  ///computes outer product xy^T into this matrix
